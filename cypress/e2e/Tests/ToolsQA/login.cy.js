...
describe("Login page", () => {
  ...
  const { username, password } = it("Login Success with Valid Credentials", () => {
  ...
  it("Login fail with Invalid Credentials", () => {
    cy.fixture("data/loginCredentials").then((data) => {
      ...
      LoginPage.clickLoginButton();
      LoginPage.elements.invalidCredentialsPopup().should("be.visible").and("contain", "Invalid credentials");
    });
  });
});
